					...
					<form method="POST" action="user_aboutme_php.php" enctype="multipart/form-data">
						<div class="row" style="margin-bottom: 3rem;">
							...
							<?php
								...
								$select_user_total_jobs = "SELECT * FROM job_post WHERE arbeitgeber_email='".$arbeitgeber_email."' ORDER BY job_post_id DESC";
								$selected_user_total_jobs = mysqli_query($conn,$select_user_total_jobs);
									while($row=mysqli_fetch_assoc($selected_user_total_jobs))
									{
										$select_total_job_likes = "SELECT * FROM job_action WHERE job_post_id='".$row['job_post_id']."' ORDER BY job_post_id DESC";
										$selected_total_job_likes = mysqli_query($conn,$select_total_job_likes);
											while($row2=mysqli_fetch_assoc($selected_total_job_likes))
											{
							?>
												<div class="col-sm-6 col-md-4 mb-3">
													<div class="jobbox" id="<?php echo($row['job_post_id']); ?>">
														<img src="Job_Post_img/<?php echo($row['job_post_image']); ?>" alt="job_img" class="job_img">
														<div class="job_detail" style="cursor: default;">
															<div class="job_text">
																<i class="fa fa-thumbs-up" aria-hidden="true" id="total_job_likes"></i><h6 id="total_job_likes_numbers"><?php echo($row2['likes']); ?></h6>
															</div>
														</div>
													</div>
												</div>
